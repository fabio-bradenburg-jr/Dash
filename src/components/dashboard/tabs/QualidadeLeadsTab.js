...
/* ── Classificação por nº de dias com leads de boa qualidade ── */
function colorForCount(c) { return c >= 5 ? C.green : c === 4 ? C.blue : c === 3 ? C.yellow : C.red }
function statusTextForCount(c) { return c >= 5 ? 'Excelente' : c === 4 ? 'Boa qualidade' : c === 3 ? 'Atenção' : 'Baixa qualidade' }
...
export default function QualidadeLeadsTab({ clients = [], workspaceUsers = [], currentUserId, isMaster }) {
  ...
  const [statusFilter, setStatusFilter] = useState('all')       // all | green | blue | yellow | red
  ...
  /* janela de semanas carregada (para dashboard/evolução) */
  const HEAT_WEEKS = 6
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/client-lead-quality?from=${from}&to=${to}`, { cache: 'no-store' })
  ...
  /* conta dias com leads de boa qualidade (Seg-Sex) numa semana, respeitando filtro de responsável */
  ...
  const toggleDay = useCallback(async (clientId, day) => {
    ...
    setSaving(true)
    try {
      await fetch('/api/client-lead-quality', {
    ...
    <div style={{ width: '100%', color: C.text, fontFamily: "'Plus Jakarta Sans',system-ui,sans-serif" }}>
      <div style={{ maxWidth: 1440, margin: '0 auto', padding: '0 0 28px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontFamily: 'Inter', fontSize: '0.62rem', textTransform: 'uppercase', letterSpacing: '0.12em', color: C.accent, fontWeight: 700, marginBottom: 9 }}>
                <i className="bx bx-target-lock" style={{ fontSize: 15 }} />Verificação diária de leads
              </div>
              <h1 style={{ margin: 0, fontWeight: 800, fontSize: 'clamp(1.5rem,2.6vw,2rem)', letterSpacing: '-0.02em', lineHeight: 1.05 }}>Qualidade dos Leads</h1>
              <p style={{ margin: '8px 0 0', color: C.text2, fontSize: '0.88rem', maxWidth: '60ch', lineHeight: 1.5 }}>Acompanhe diariamente se os leads de cada cliente estão vindo com boa qualidade. Marque Sim/Não por dia e veja a consistência semanal da carteira.</p>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 12, marginTop: 16 }}>
          ...
          <StatCard icon="bx-check-double" color={C.accentBright} label="Dias positivos" value={dash.total} sub="total no período" />
          <StatCard icon="bx-star" color={C.green} label="Excelente" value={dash.green} sub="clientes no verde" />
          <StatCard icon="bx-check-circle" color={C.blue} label="Boa qualidade" value={dash.blue} sub="clientes no azul" />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 12, marginTop: 12 }}>
          <HighlightCard icon="bx-crown" color={C.green} title="Melhor qualidade" client={dash.topC?.c} pct={dash.topC?.pct} />
          <HighlightCard icon="bx-down-arrow-alt" color={C.red} title="Pior qualidade" client={dash.bottomC?.c} pct={dash.bottomC?.pct} />
          ...
          <div style={{ overflowX: 'auto' }}>
            <div style={{ minWidth: periodMode === 'week' ? 860 : 720 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: periodMode === 'week' ? '2fr repeat(5, 0.7fr) 1.4fr 1.2fr' : `2fr repeat(${monthWeeks.length || 1}, 0.7fr) 1.4fr`, gap: 8, padding: '11px 16px', borderBottom: `1px solid ${C.border}`, background: 'rgba(255,255,255,0.02)', fontFamily: 'Inter', fontSize: '0.56rem', textTransform: 'uppercase', letterSpacing: '0.06em', color: C.text3, fontWeight: 700 }}>
                <span>Cliente</span>
                {periodMode === 'week'
                  ? (<>{DIAS.map(d => <span key={d.key} style={{ textAlign: 'center' }}>{d.short}</span>)}<span style={{ textAlign: 'center' }}>Qualidade</span><span style={{ textAlign: 'center' }}>Status</span></>)
                    ...
                    {periodMode === 'week' ? (
                      <>
                        {DIAS.map(d => {
                          ...
                          const who = entry ? `${entry.byName || 'Equipe'}${entry.at ? ' · ' + new Date(entry.at).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : ''}` : 'Marcar leads como bons'
                          return (
